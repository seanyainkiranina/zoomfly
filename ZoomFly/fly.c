...
int         process_args(int argc, wchar_t *argv[]);
...
int main(int argc, char *argv[]) {
	...
	char *font = (char *)my_newmem(6);
	sprintf(font, "%s", "times");
	int brect[8];
	double curang = 0.0;
	char *error_report;
...
START:	type = get_token(infile);
	...
	do {
		...
		case SETFONT:
			font = get_string(infile);
			if (!quiet) fprintf(verbose_out, "## Setfont ##  %s. \n", font);
			break;
		case FREETYPE:
			for (i = 1; i <= 3; i++) {
			...
			s = get_string(infile);
			//  size  x y colour text
			error_report = gdImageStringFT(img, brect, colour, font, arg[1], curang, arg[2], arg[3], s);
			if (!quiet) fprintf(verbose_out, "## ftfont ##  %s %s \n", font, s);
			if (error_report)
...
int get_token(FILE *infile) {
	...
	if (strcmp(input_type, freetype) == 0) {
		free(input_type);
		return FREETYPE;
	}
	if (strcmp(input_type, setfont) == 0) {
		free(input_type);
		return SETFONT;
	}
...
void copy_to_img(FILE *infile, gdImagePtr img, int resize) {
	...
	if (img_file == NULL) {
		...
		if (!quiet) fprintf(verbose_out, "Could not read in image\n");
	...
	if (resize == 1)
	{
		if ((arg[0] == -1) && (arg[1] == -1) &(arg[2] == -1) && (arg[3] == -1)) {
			...
			// gdImagePaletteToTrueColor(img_file);
...
{
	int c = 0, errflag = 0, option_index = 0;

	// extern wchar_t *optarg;
	extern char *optarg_a;
	extern int optind;
	if (argc == 1) {
		fprintf(stdout, "fly, version %s\n\n%s\n", version, help);
		exit(0);
	...
	while ((c = getopt(argc, argv, "qhvi:o:")) != EOF) {
		...
		case 'h': fprintf(stdout, "fly, version %s\n\n%s\n", version, help);
			exit(0);
			break;
		case 'o': output_file = (char *)my_newmem(strlen(optarg_a) * sizeof(char));
			sprintf(output_file, "%s", optarg_a);
			break;
		case 'i': input_file = (char *)my_newmem(strlen(optarg_a) * sizeof(char));
			sprintf(input_file, "%s", optarg_a);
			break;
		case '?': errflag = 1;
			break;
		case EOF: errflag = 1;
...
FILE *get_file(int *line_number, char **filename)
{
	...
	char *pfound = strstr(fname, ".gif"); //pointer to the first character found 's' in the string saux
	if (pfound != NULL)
	{
